...
void Plotter::mark_position(Vector2d<double> position, std::uint8_t red, std::uint8_t green, std::uint8_t blue){
    ...
    if (position.x < plot_bounding_box.x_min || position.x > plot_bounding_box.x_max ||
       position.y < plot_bounding_box.y_min || position.y > plot_bounding_box.y_max) {
    ...
    std::uint32_t x = static_cast<std::uint32_t>((position.x - plot_bounding_box.x_min) * scale_x);
    std::uint32_t y = static_cast<std::uint32_t>((position.y - plot_bounding_box.y_min) * scale_y);
    // Pixel markieren
